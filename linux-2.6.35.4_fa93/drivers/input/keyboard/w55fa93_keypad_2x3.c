/* linux/driver/input/w55fa93_keypad_2x3.c
 *
...
const int key_map[KEY_COUNT] = {        
	ASC_KEY_UP, ASC_KEY_LEFT, ASC_KEY_ESC, ASC_KEY_DOWN, ASC_KEY_RIGHT, ASC_KEY_ENTER
};
...
//	  printk("->0\n");
	...
	read0 = new_key;
	writel(readl(REG_GPIOA_DOUT) |(1 << 2), REG_GPIOA_DOUT); // GPA2 high
	
	read1 = readl(REG_GPIOA_PIN) & 0xE0;
	
	if(read1 == 0xE0)
		new_key = (read0 ^ read1)>>5;
	else
...
static int __init w55fa93_kpd_reg(void) {
        ...
        writel(readl(REG_IRQSRCGPA) & ~(0xFC00), REG_IRQSRCGPA); // GPA[5~7] as nIRQ0 source
	writel((readl(REG_IRQENGPA)& ~(0x00E00000)) | 0x00000E0, REG_IRQENGPA); // falling edge trigger
	writel((readl(REG_AIC_SCR1)& ~(0x00C70000)) | 0x00470000, REG_AIC_SCR1);
        
        // PORT A[2 ]
        writel(readl(REG_GPIOA_OMD) | (1 << 2), REG_GPIOA_OMD);  // output
