...
extern unsigned int w55fa93_upll_clock;
	...
	//   	outl( (inl(REG_LCM_LCDCCtl)& ~LCDCCtl_PRDB_SEL) | (0x01 << 20), REG_LCM_LCDCCtl);	// 18-bit mode 
		...
		*/
		...
		outl((inl(REG_LCM_TCON1)& 0xFF000000)|(0x00012606), REG_LCM_TCON1);
		
		// set Vertical scanning line timing for Syn type LCD   
		//outl((inl(REG_LCM_TCON2)& 0xFF000000)|(0x00022B07), REG_LCM_TCON2);
		outl((inl(REG_LCM_TCON2)& 0xFF000000)|(0x00010708), REG_LCM_TCON2);
...
static int w55fa93fb_ioctl_device(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		//	outl(inl(REG_CLKDIV1) | (2<<3), REG_CLKDIV1);					// predivider for clock from APLL or UPLL		
		//	outl((inl(REG_CLKDIV1) & ~VPOST_N1) | (0x0a << 8), REG_CLKDIV1);	// divider value = 0x0A (temp)
