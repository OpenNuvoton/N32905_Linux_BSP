						  ...
						  struct nand_flash_dev *type)
{
	...
	*maf_id = chip->read_byte(mtd);
	...
	if (!type->pagesize) {
        ...
        } else if ( id_data[0] == NAND_MFR_TOSHIBA && id_data[1] == 0xD5 && id_data[4] != 0x00 ) {
            /* Calc pagesize */
            mtd->writesize = 2048 << (extid & 0x03);
            extid >>= 2;

            /* OOB is 472B per 8KiB pagesize */
            mtd->oobsize = 448;
            extid >>= 3;
	...
	printk("Probe chip: writesize=%d erasesize=%d, oobsize=%d\n", mtd->writesize, mtd->erasesize, mtd->oobsize);
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	printk("chip->ecc.bytes = 0x%x !!!!\n", chip->ecc.bytes);		// mhkuo
