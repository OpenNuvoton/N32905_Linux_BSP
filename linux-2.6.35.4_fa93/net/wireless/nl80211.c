...
static int nl80211_get_key(struct sk_buff *skb, struct genl_info *info)
{
    ...
    // mtk_p2p_cfg80211_get_key of MT5931_MP_Linux_P2P_W1233_V1.0 infact doesn't use bool pairwise,
    // just set the pairwirse to true and let it pass the compile.
	// err = rdev->ops->get_key(&rdev->wiphy, dev, key_idx, mac_addr,
	//			&cookie, get_key_callback);
	err = rdev->ops->get_key(&rdev->wiphy, dev, key_idx, true, mac_addr,
				&cookie, get_key_callback);
    //End Add by Henry
	if (err)
		goto free_msg;
...
static int nl80211_new_key(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!err)
		...
		//		     mac_addr, &key.p);
		err = rdev->ops->add_key(&rdev->wiphy, dev, key.idx, true,
					 mac_addr, &key.p);
        //End Add by Henry
	wdev_unlock(dev->ieee80211_ptr);
...
static int nl80211_del_key(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!err)
        ...
        err = rdev->ops->del_key(&rdev->wiphy, dev, key.idx, true, mac_addr);
        //End Add by Henry
#ifdef CONFIG_CFG80211_WEXT
	if (!err) {
